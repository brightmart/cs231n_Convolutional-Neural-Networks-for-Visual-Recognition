...
class PretrainedCNN(object):
  def __init__(self, dtype=np.float32, num_classes=100, input_size=64, h5_file=None):
    ...
    self.conv_params.append({'stride': 2, 'pad': 2}) #0
    self.conv_params.append({'stride': 1, 'pad': 1}) #1
    self.conv_params.append({'stride': 2, 'pad': 1}) #2
    self.conv_params.append({'stride': 1, 'pad': 1}) #3
    self.conv_params.append({'stride': 2, 'pad': 1}) #4
    self.conv_params.append({'stride': 1, 'pad': 1}) #5
    self.conv_params.append({'stride': 2, 'pad': 1}) #6
    self.conv_params.append({'stride': 1, 'pad': 1}) #7
    self.conv_params.append({'stride': 2, 'pad': 1}) #8

    self.filter_sizes = [5, 3, 3, 3, 3, 3, 3, 3, 3]
    self.num_filters = [64, 64, 128, 128, 256, 256, 512, 512, 1024] #CHANGE.2016.8.21.last input orginal is ...,512, 512, 1024],but in my computer i change it to a very small number. as 64
    hidden_dim = 512 #orginal is 512,but in my computer i change it to a very small number. as 64
    ...
    for i, (f, next_dim) in enumerate(zip(self.filter_sizes, self.num_filters)):
      fan_in = f * f * prev_dim
      self.params['W%d' % (i + 1)] = np.sqrt(2.0 / (fan_in+0.00001)) * np.random.randn(next_dim, prev_dim, f, f)
  ...
  def forward(self, X, start=None, end=None, mode='test'): #read 1--->1.load parameters of each function,2.invoke function,3.update input X,4.loop layers until reach end point.
    """
    ...
    for i in xrange(start, end + 1):
      ...
      if 0 <= i < len(self.conv_params):
        print "i:",i,"a conv layer. end:",end
  ...
  def backward(self, dout, cache): #read 2--->1. call backward function of each layers to get parameters. 2.save each parameters.3.update gradient of the layer.4.move to previous layers.
    """
  ...
  def loss(self, X, y=None): #read 3. call forward ( and backward) function to get score ( and gradients)
    """
